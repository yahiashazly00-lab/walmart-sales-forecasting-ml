"""
Walmart Weekly Sales Predictor — Streamlit App (v8)
====================================================
Supports FOUR prediction pipelines:
  ...
  • XGBoost            → loads xgboost_model.pkl + xgboost_feature_names.json
...
v8 addition (XGBoost):
  ⑪ XGBoost pipeline added (notebook Section 18).
     Uses raw/unnormalized features — no scaler needed (tree-based, scale-invariant).
     Pipeline key: 'xgb'. Files: xgboost_model.pkl + xgboost_feature_names.json.
     Gradient-boosted ensemble; generally highest accuracy on this dataset.
"""
...
def load_pipelines():
    ...
    # ── XGBoost pipeline ─────────────────────────────────────────────────────
    xgb_model_path = os.path.join(d, 'xgboost_model.pkl')
    xgb_feat_path  = os.path.join(d, 'xgboost_feature_names.json')
    # XGBoost uses raw features — no scaler needed (tree splits are rank-order)

    try:
        try:
            import xgboost  # noqa: F401  — required to unpickle XGBRegressor
        except ImportError:
            raise ImportError(
        ...
        if not os.path.exists(xgb_model_path):
            raise FileNotFoundError(
                f"xgboost_model.pkl not found in {d}.\n"
                "Run notebook Section 18.5 to train and save the XGBoost model."
            )
        if not os.path.exists(xgb_feat_path):
            raise FileNotFoundError(f"xgboost_feature_names.json not found in {d}.")

        xgb_mdl = joblib.load(xgb_model_path)
        with open(xgb_feat_path) as f_xgb:
            xgb_features = json.load(f_xgb)

        pipelines['xgb'] = {
            'model': xgb_mdl, 'scaler': None, 'features': xgb_features,
            'label': 'XGBoost', 'error': None, 'model_file': 'xgboost_model.pkl',
        }
    except Exception as e:
        pipelines['xgb'] = {
            'model': None, 'scaler': None, 'features': None,
            'label': 'XGBoost', 'error': str(e), 'model_file': '—',
        }
...
xgb_ok  = pipelines['xgb']['error']  is None
any_ok  = norm_ok or raw_ok or lr_ok or xgb_ok
...
st.markdown("""
<div class="hero">
    <div class="hero-tag">🌲 Random Forest · 📐 Linear Regression · 🚀 XGBoost · Quad Pipeline</div>
...
if xgb_ok:
    n_feat = len(pipelines['xgb']['features'])
    chips_html += f'<span class="chip" style="border-color:#6a1b9a;color:#ce93d8;background:#0f0a1a;">🟣 XGBoost loaded · {n_feat} features</span>'
else:
    chips_html += '<span class="chip chip-warn">⚠️ XGBoost unavailable</span>'
chips_html += '</div>'
...
if not any_ok:
    ...
    &nbsp;&nbsp;• <code>feature_names_linear.json</code>&nbsp;&nbsp;&nbsp;(Section 17)<br>
    &nbsp;&nbsp;• <code>xgboost_model.pkl</code>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(Section 18.5)<br>
    &nbsp;&nbsp;• <code>xgboost_feature_names.json</code>&nbsp;&nbsp;(Section 18.5)
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
if xgb_ok:  available_options.append("🟣 XGBoost  (raw feature values · gradient-boosted ensemble)")
...
is_xgb   = (pipeline_key == 'xgb')
is_norm_or_lr = is_norm or is_lr

if is_xgb:
    badge_color  = "#6a1b9a"
    badge_label  = "XGBoost"
    scaler_status = "ℹ️  No scaler — raw values sent directly to model (tree-based)"
elif is_lr:
    badge_color  = "#2e7d32"
...
<div class="badge" style="border-color:{badge_color};">
    <strong style="color:{'#90caf9' if is_norm else ('#a5d6a7' if is_lr else ('#ce93d8' if is_xgb else '#ffcc80'))};">{badge_label} Pipeline</strong>
    &nbsp;·&nbsp; {scaler_status}
    ...
    <div class="card">
        ...
        <table style="width:100%;font-size:0.83rem;border-collapse:collapse;">
            ...
            <tr><td style="color:#8ba8cc;padding:0.2rem 0;">Pipeline</td>
                <td style="color:{'#90caf9' if is_norm else ('#a5d6a7' if is_lr else ('#ce93d8' if is_xgb else '#ffcc80'))};text-align:right;font-weight:600;">{badge_label}</td></tr>
        </table>
    </div>
    """, unsafe_allow_html=True)
...
<div style="text-align:center;font-size:0.74rem;color:#2d4a6e;padding:1.5rem 0 0.5rem;">
    Walmart Sales Predictor v8 · Quad Pipeline · Random Forest + Linear Regression + XGBoost · scikit-learn · xgboost
</div>
""", unsafe_allow_html=True)
